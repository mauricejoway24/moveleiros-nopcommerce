...
namespace Nop.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual IPagedList<Product> SearchProducts(
            ...
            IList<int> cityList = null,
            decimal? areaFrom = null,
            decimal? areaTo = null,
            IList<int> brandIds = null,
            IList<int> allowedStoreList = null)
        {
            ...
            return SearchProducts(out filterableSpecificationAttributeOptionIds, false,
                ...
                orderBy, showHidden, overridePublished, cityList, areaFrom, areaTo, brandIds,
                allowedStoreList);
        }
            ...
            IList<int> allowedStoreList = null)
        {
            ...
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                #region Store limit (Moveleiros)

                // all users appended to a unique store or multiple store shouldn't be passed throw StoreId
                // that being said, use "StoreIds" instead which supports multiple store
                var commaSeparatedAllowedStores = string.Empty;
                if (allowedStoreList != null && allowedStoreList.Count > 0)
                {
                    commaSeparatedAllowedStores = string.Join(",", allowedStoreList);
                }

                var pStoreIds = _dataProvider.GetParameter();
                pStoreIds.ParameterName = "StoreIds";
                pStoreIds.Value = commaSeparatedAllowedStores;
                pStoreIds.DbType = DbType.String;
                ...
                var pAreaFrom = _dataProvider.GetParameter();
                pAreaFrom.ParameterName = "AreaFrom";
                pAreaFrom.Value = priceMin.HasValue ? (object)areaFrom.Value : DBNull.Value;
                pAreaFrom.DbType = DbType.Decimal;

                var pAreaTo = _dataProvider.GetParameter();
                pAreaTo.ParameterName = "AreaTo";
                pAreaTo.Value = priceMin.HasValue ? (object)areaTo.Value : DBNull.Value;
                pAreaTo.DbType = DbType.Decimal;
                ...
                pCity.ParameterName = "CityIds";
                if (cityList != null && cityList.Any())
                    pCity.Value = string.Join(",", cityList);
                else
                    pCity.Value = "";
                pCity.DbType = DbType.String;

                var pBrand = _dataProvider.GetParameter();
                pBrand.ParameterName = "BrandIds";
                if (brandIds != null && brandIds.Any())
                    pBrand.Value = string.Join(",", brandIds);
                else
                    pBrand.Value = "";
                pBrand.DbType = DbType.String;
                ...
                if (cityList != null && cityList.Any())
                {
                    ...
                    query = from p in query
                            join sm in _storeMappingRepository.Table
                                on new { c1 = p.Id, c2 = "Product" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into p_sm
                            from sm in p_sm.DefaultIfEmpty()
                            join st in storeRepository.Table
                                on sm.StoreId equals st.Id
                            where (!p.LimitedToStores || storeId == sm.StoreId) &&
                                cityList.Contains(st.CityId)
                            select p;
                }
        ...
        #region Moveleiros Product Colors

        /// <summary>
        /// Deletes a product picture
        /// </summary>
        /// <param name="productPicture">Product picture</param>
        public virtual void DeleteProductColorPicture(ProductColor productPicture)
        {
            if (productPicture == null)
                throw new ArgumentNullException("productPicture");

            _productColorRepository.Delete(productPicture);
        ...
        /// <param name="productId">The product identifier</param>
        /// <returns>Product pictures</returns>
        public virtual IList<ProductColor> GetProductColorPicturesByProductId(int productId)
        {
            var query = from pp in _productColorRepository.Table
                        where pp.ProductId == productId
                        select pp;

            query = (query.Where(t => t.Picture.PictureTypeId == (int)Core.Domain.Media.PictureType.Color))
                .OrderBy(t => new { t.DisplayOrder, t.Id });
        ...
        /// <param name="productPictureId">Product picture identifier</param>
        /// <returns>Product picture</returns>
        public virtual ProductColor GetProductColorPictureById(int productPictureId)
        {
            if (productPictureId == 0)
                return null;

            return _productColorRepository.GetById(productPictureId);
        }
        ...
        /// </summary>
        /// <param name="productPicture">Product picture</param>
        public virtual void InsertProductColorPicture(ProductColor productColor)
        {
            if (productColor == null)
                throw new ArgumentNullException("productColor");

            _productColorRepository.Insert(productColor);

            //event notification
            _eventPublisher.EntityInserted(productColor);
        }
        ...
        /// </summary>
        /// <param name="productPicture">Product picture</param>
        public virtual void UpdateProductColorPicture(ProductColor productPicture)
        {
            if (productPicture == null)
                throw new ArgumentNullException("productPicture");

            _productColorRepository.Update(productPicture);
        ...
        /// <param name="productsIds">Products IDs</param>
        /// <returns>All picture identifiers grouped by product ID</returns>
        public IDictionary<int, int[]> GetProductColorsImagesIds(int[] productsIds)
        {
            return _productPictureRepository.Table.Where(p => productsIds.Contains(p.ProductId))
                .GroupBy(p => p.ProductId).ToDictionary(p => p.Key, p => p.Select(p1 => p1.PictureId).ToArray());
        }
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (storeId != 0)
            {
                ...
                query = from p in query
                        join sm in _storeMappingRepository.Table
                        on new { c1 = p.ProductId, c2 = "Product" } equals new { c1 = sm.EntityId, c2 = sm.EntityName } into p_sm
                        from sm in p_sm.DefaultIfEmpty()
                        where storeId.Equals(sm.StoreId)
                        select p;
            }
            #endregion
