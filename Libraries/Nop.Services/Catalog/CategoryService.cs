...
namespace Nop.Services.Catalog
{
    ...
    public partial class CategoryService : ICategoryService
    {
        ...
        private const string CATEGORIES_PARENT_BY_CHILDREN_CATEGORY_ID_KEY = "Nop.categoryparent.bychilden-{0}-{1}-{2}";
        /// <summary>
        /// Key for caching
        /// </summary>
        /// <remarks>
        ...
        public virtual IPagedList<Category> GetAllCategories(string categoryName = "", int storeId = 0, 
            int pageIndex = 0, int pageSize = int.MaxValue, bool showHidden = false, bool onlySubCategory = false,
            bool onlyCategory = false)
        {
           ...
           #region Extensions by QuanNH
            else
            {
                ...
                if (onlySubCategory)
                    query = query.Where(t => t.ParentCategoryId > 0);
        ...
        public IList<Category> GetAllCategoriesParentByChildrenId(IList<int> childrenIds)
        {
            string key = string.Format(CATEGORIES_PARENT_BY_CHILDREN_CATEGORY_ID_KEY, string.Join(",", childrenIds), 
            ...
            return _cacheManager.Get(key, () =>
            {
                var query = _categoryRepository.Table;
                query = query.Where(c => c.Published);
                query = query.Where(c => childrenIds.Contains(c.Id));
                query = query.Where(c => !c.Deleted);
                query = query.OrderBy(c => c.DisplayOrder).ThenBy(c => c.Id);

                if (!_catalogSettings.IgnoreAcl || !_catalogSettings.IgnoreStoreLimitations)
                {
                    if (!_catalogSettings.IgnoreAcl)
                    {
                        //ACL (access control list)
                ...
                var categories = query.ToList();
                var categoriesIds = categories.Select(t => t.Id);

                // Only parents...
                query = _categoryRepository.Table;

                query = query.Where(c => categoriesIds.Contains(c.Id));

                query = from c in query
                        join parent in _categoryRepository.Table
                        on c.ParentCategoryId equals parent.Id
                        orderby parent.DisplayOrder, parent.Id
                        select parent;
