...
namespace Nop.Admin.Controllers
{
    public partial class CustomerController : BaseAdminController
    {
        ...
        protected virtual void PrepareCustomerModel(CustomerModel model, Customer customer, bool excludeProperties)
        {
            ...
            if (customer != null)
            {
                ...
                if (!excludeProperties)
                {
                    ...
                    {
                        // Removed by Moveleiros - TOOOOO SLOW
            ...
            foreach (var role in allRoles)
            {
                ...
                var isInRoleStore = _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Stores);

                if (isInRoleStore && 
                    !SystemCustomerRoleNames.GetStoreOnlyPermission().Contains(role.SystemName))
                    continue;

                if (_workContext.CurrentCustomer.IsVendor() && 
                    !isInRoleStore &&
                    !SystemCustomerRoleNames.GetVendorRolePermission().Contains(role.SystemName))
                    continue;
        ...
        public virtual ActionResult List()
        {
            ...
            var isCurrentUserStore = _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Stores) ||
                _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Vendors);

            var allRoles = _customerService.GetAllCustomerRoles(!isCurrentUserStore);

            if (isCurrentUserStore)
                allRoles = allRoles
                    .Where(t => SystemCustomerRoleNames.GetStoreOnlyPermission().Contains(t.SystemName))
                    .ToList();

            if (_workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Vendors))
                allRoles = allRoles
                    .Where(t => SystemCustomerRoleNames.GetVendorRolePermission().Contains(t.SystemName))
                    .ToList();
            ...
            [ModelBinder(typeof(CommaSeparatedModelBinder))] int[] searchCustomerRoleIds)
        {
            ...
            if (searchCustomerRoleIds.Count() == 0)
            {
                var isCurrentUserStore = _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Stores) ||
                    _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Vendors);
                ...
                if (allRoles.Count > 0)
                    searchCustomerRoleIds = allRoles.Select(t => t.Id).ToArray();
            }
        ...
        public virtual ActionResult Create(CustomerModel model, bool continueEditing, FormCollection form)
        {
            ...
            if (!String.IsNullOrWhiteSpace(model.Email))
            {
                ...
                if (cust2 != null)
                    ModelState.AddModelError("", "Email já utilizado.");
            }
            if (!String.IsNullOrWhiteSpace(model.Username) & _customerSettings.UsernamesEnabled)
            {
                var cust2 = _customerService.GetCustomerByUsername(model.Username);
                if (cust2 != null)
                    ModelState.AddModelError("", "Nome de usuário já utilizado.");
            }


            #region Customer roles validation

            //validate customer roles
            var allCustomerRoles = _customerService.GetAllCustomerRoles(true);

            AddRegisteredUserWhenAdminStore(model, allCustomerRoles);
            ...
            if (ModelState.IsValid)
            {
                ...
                #region Adjust customer store

                if (_workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Stores))
                {
                    var storeMapping = Core.Infrastructure.EngineContext.Current.Resolve<IStoreMappingService>();
                    var getCurrentStore = storeMapping.CurrentStore();

                    if (getCurrentStore > 0)
                    {
                        storeMapping.InsertStoreMapping(customer.Id, "Stores", getCurrentStore);
                    }
                }
        ...
        private void AddRegisteredUserWhenAdminStore(CustomerModel model, IList<CustomerRole> allCustomerRoles)
        {
            // Moveleiros
            // When user is AdministratorStore, add user as Registered
            if (_workContext.IsAdmin)
            {
                var registeredRole = allCustomerRoles
                    .Where(t => t.SystemName == SystemCustomerRoleNames.Registered)
                    .FirstOrDefault();
