...
namespace Nop.Admin.Controllers
{
    public partial class StoreController : BaseAdminController
    {
        ...
        protected virtual void PrepareCitiesModel(StoreModel model)
        {
            if (model == null)
                throw new ArgumentNullException("model");
            ...
            foreach (var city in cities)
            {
                model.AvailableCities.Add(new SelectListItem
                {
                    Text = string.Format("{0}-{1}", city.Description, city.UF),
        ...
        [HttpPost]
        public virtual ActionResult List(DataSourceRequest command, StoreModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageStores))
                return AccessDeniedKendoGridJson();

            var storeModels = _storeService.GetPagedStores(
                new Store
                {
                    Name = model.Name,
                    CompanyAddress = model.CompanyAddress,
                    LojistaId = model.LojistaId
                }, 
                command.Page, 
                command.PageSize
            );
        ...
        public virtual ActionResult Create(StoreModel model, bool continueEditing)
        {
            ...
            if (_storeService.GetStoreByLojistaId(model.LojistaId) != null)
            {
                ModelState.AddModelError("LojistaIdExist", "Cód do Lojista já cadastrado");
            }

            if (ModelState.IsValid)
            {
                var companyNameUrl = model.CompanyName
                    .Trim()
                    .ToLower()
                    .Replace(" ", "-")
                    .Replace("\\", "");
                ...
                // Urls
                model.ProfileShortUrl = $"{companyNameUrl}-{model.LojistaId}";
                model.Url = $"http://{model.ProfileShortUrl}.moveleiros.com.br/";
                model.SecureUrl = $"https://{model.ProfileShortUrl}.moveleiros.com.br/";
                model.Hosts = $"{model.ProfileShortUrl}.moveleiros.com.br";
        ...
        public virtual ActionResult Edit(StoreModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                // Prevent lojistaId to be changed by now
                model.LojistaId = store.LojistaId;
