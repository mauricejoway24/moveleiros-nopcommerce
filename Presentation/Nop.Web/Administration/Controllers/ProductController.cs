...
namespace Nop.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        protected virtual void PrepareStoresMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            ...
            if (storeId.Count > 0)
            {
                allStores = _storeService.GetAllStoresByEntityName(_workContext.CurrentCustomer.Id, "Stores");
            }
            ...
            if (!excludeProperties)
            {
                ...
                else
                {
                    if (storeId.Count > 0)
                    {
                        model.SelectedStoreIds = storeId;
        ...
        protected virtual void PrepareCategoryMappingModel(ProductModel model, Product product, bool excludeProperties)
        {
            ...
            if (!excludeProperties && product != null)
                model.SelectedCategoryIds = _categoryService
                    .GetProductCategoriesByProductId(product.Id, true)
                    .Select(c => c.CategoryId).ToList();

            var allCategories = SelectListHelper.GetCategoryList(_categoryService, _cacheManager, true, true);
        ...
        {
            var productCategoriesParent = _categoryService.GetAllCategoriesParentByChildrenId(model.SelectedCategoryIds);
            var existingProductCategories = _categoryService.GetProductCategoriesByProductId(product.Id, true);

            foreach (var item in productCategoriesParent)
            {
                model.SelectedCategoryIds.Add(item.Id);
            }
        ...
        protected virtual void PrepareProductModel(ProductModel model, Product product, bool setPredefinedValues, bool excludeProperties)
        {
            ...
            // Check if user is Moveleiros :)
            model.IsAdminSupreme = _workContext.CurrentCustomer.IsInCustomerRole(SystemCustomerRoleNames.Administrators);

            if (!model.DimensionImageId.HasValue)
                model.DimensionImageId = 0;
            ...
            {
                Text = _localizationService.GetResource("Moveleiros.Admin.Catalog.Products.Fields.Brand.None"),
                Value = "0"
            });
            var brands = SelectListHelper.GetBrandList(_brandService, _cacheManager, true);
            foreach (var b in brands)
                model.AvailableBrands.Add(b);
        ...
        public virtual ActionResult ProductList(DataSourceRequest command, ProductListModel model)
        {
            ...
            // should find only products allowed by a list of store
            var allowedStoreIds = new List<int>();

            var allStores = _storeService.GetAllStoresByEntityName(_workContext.CurrentCustomer.Id, "Stores");
            if (allStores.Count > 0)
            {
                allowedStoreIds = allStores.Select(t => t.Id).ToList();
            }
            ...
            var products = _productService.SearchProducts(
                ...
                allowedStoreList: allowedStoreIds
            );
        ...
        public virtual ActionResult Create()
        {
            ...
            // Moveleiros
            if (CurrentStoreNotAutorizedToPublish())
            {
                model.ProductEditorSettingsModel.Published = false;
                model.Published = false;
            }
        ...
        private bool CurrentStoreNotAutorizedToPublish() => _storeMappingService.CurrentStore() != 0;
        ...
        public virtual ActionResult Edit(int id)
        {
            ...
            if (!_storeMappingService.TableEdit(product.Id, "Product"))
                return RedirectToAction("List");
        ...
        public virtual ActionResult Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (product.DimensionImageId == 0)
                    product.DimensionImageId = null;
            ...
            int productId, bool isPanorama)
        {
            ...
            _pictureService.SetSeoFilename(pictureId, _pictureService.GetPictureSeName(product.Name), isPanorama: isPanorama);
        ...
        #region Moveleiros Product Color

        [ValidateInput(false)]
        public virtual ActionResult ProductColorAdd(int pictureId, 
            int displayOrder,
            string overrideAltAttribute, 
            ...
            int productId)
        {
            ...
            _pictureService.UpdatePicture(picture.Id,
                ...
                overrideAltAttribute,
                overrideTitleAttribute,
                pictureType: PictureType.Color);

            _pictureService.SetSeoFilename(
                pictureId, 
                _pictureService.GetPictureSeName(product.Name), 
                pictureType: PictureType.Color
            );

            _productService.InsertProductColorPicture(new ProductColor
            {
                PictureId = pictureId,
                ProductId = productId,
                DisplayOrder = displayOrder,
        ...
        [HttpPost]
        public virtual ActionResult ProductColorList(DataSourceRequest command, int productId)
        {
            ...
            if (_workContext.CurrentVendor != null)
            {
                ...
                if (product != null && product.VendorId != _workContext.CurrentVendor.Id)
                {
                    return Content("Não é seu produto");
                }
            }

            var productColorPictures = _productService.GetProductColorPicturesByProductId(productId);
            var productPicturesModel = productColorPictures
                .Select(x =>
                {
                    var picture = _pictureService.GetPictureById(x.PictureId);
                    ...
                    var m = new ProductModel.ProductPictureModel
                    {
                        ...
                        ProductId = x.ProductId,
                        PictureId = x.PictureId,
                        PictureUrl = _pictureService.GetPictureUrl(picture, defaultPictureType: PictureType.Color),
                        OverrideAltAttribute = picture.AltAttribute,
                        OverrideTitleAttribute = picture.TitleAttribute,
                        DisplayOrder = x.DisplayOrder
                    };
        ...
        [HttpPost]
        public virtual ActionResult ProductColorUpdate(ProductModel.ProductPictureModel model)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();

            var productPicture = _productService.GetProductColorPictureById(model.Id);
            ...
            productPicture.DisplayOrder = model.DisplayOrder;
            _productService.UpdateProductColorPicture(productPicture);

            return new NullJsonResult();
        }

        [HttpPost]
        public virtual ActionResult ProductColorDelete(int id)
        {
            if (!_permissionService.Authorize(StandardPermissionProvider.ManageProducts))
                return AccessDeniedView();
        ...
        public virtual ActionResult TierPriceList(DataSourceRequest command, int productId)
        {
            ...
            {
                #region Extensions by QuanNH
                  string storeName = _localizationService.GetResource("Admin.Catalog.Products.TierPrices.Fields.Store.All");
                #endregion
