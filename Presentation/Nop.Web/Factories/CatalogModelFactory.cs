...
namespace Nop.Web.Factories
{
    public partial class CatalogModelFactory : ICatalogModelFactory
    {
        ...
        public virtual SearchModel PrepareSearchModel(SearchModel model, CatalogPagingFilteringModel command)
        {
            ...
            string cacheCategoryKey = string.Format(ModelCacheEventConsumer.SEARCH_CATEGORIES_MODEL_KEY + "-cacheStored",
                _workContext.WorkingLanguage.Id,
                string.Join(",", _workContext.CurrentCustomer.GetCustomerRoleIds()),
                _storeContext.CurrentStore.Id);

            var categoryIds = model.selcats == null ? new List<int>() : model.selcats;
            var brandIds = model.selBrands == null ? new List<int>() : model.selBrands;

            // Deal with categories
            var storedCategories = _cacheManager.Get(cacheCategoryKey, () =>
            {
            ...
            var filteredCategories = storedCategories
                .Where(t =>
                    categoryIds.Contains(t.Id) &&
                    t.ParentCategoryId > 0
                );

            // check if any children is selected
            // if yes: categoryIds is now only children
            // if no: categoryIds stays the way it is
            ...
            var categories = _cacheManager.Get(cacheKey, () =>
            {
                ...
                //all categories
                var allCategories = storedCategories;
            ...
            var cities = cityService.SearchCities(model.City);

            decimal areaFromConverted = 0;
            decimal areaToConverted = 0;

            decimal.TryParse(model.AreaFrom, out areaFromConverted);
            decimal.TryParse(model.AreaTo, out areaToConverted);
            ...
            products = _productService.SearchProducts(
                ...
                vendorId: vendorId,
                cityList: cities,
                areaFrom: areaFromConverted,
                areaTo: areaToConverted,
                brandIds: brandIds);
