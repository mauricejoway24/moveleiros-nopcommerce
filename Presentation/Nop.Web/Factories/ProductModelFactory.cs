...
namespace Nop.Web.Factories
{
    ...
    public partial class ProductModelFactory : IProductModelFactory
    {
        ...
        protected virtual ProductDetailsModel.AddToCartModel PrepareProductAddToCartModel(Product product, ShoppingCartItem updatecartitem)
        {
            ...
            var productStore = _storeMappingService
                .GetStoreIdByEntityId(product.Id, "Product")
                .FirstOrDefault();

            model.ProductInStoreId = productStore <= 0 ? 1 : productStore;
            model.CtaCustomizationColor = _settingService.GetSettingByKey<string>("moveleiros.mov-cta-customization");
            model.CtaCustomizationColorHover = _settingService.GetSettingByKey<string>("moveleiros.mov-cta-customization.hover");
            ...
            bool forceRedirectionAfterAddingToCart = false)
        {
            ...
            foreach (var product in products)
            {
                ...
                var mappedStore = storeService.GetStoreById(model.ProductInStoreId);
                var storeCity = cityService.GetCityById(mappedStore.CityId);
                var cutCompanyName = mappedStore.CompanyName.Split(new char[] { ' ' }, 2, StringSplitOptions.RemoveEmptyEntries);
                model.StoreName = cutCompanyName.Length == 2 ? string.Join(" ", cutCompanyName) : mappedStore.CompanyName;
                model.StoreShortAddress = storeCity == null ? "" : $"{storeCity.Description} - {storeCity.UF}";
            ...
            ShoppingCartItem updatecartitem = null, bool isAssociatedProduct = false)
        {
            ...
            var model = new ProductDetailsModel
            {
                ...
                ProjectContains = product.ProjectContains,
                AsideObservation = product.AsideObservation,
           ...
           //pictures
            ...
            // colors
            var colorPictureModels = _productService.GetProductColorPicturesByProductId(product.Id);
            model.ColorPictureModels = new List<PictureModel>();
            foreach (var colorPic in colorPictureModels)
            {
                var c = _pictureService.GetPictureById(colorPic.PictureId);

                model.ColorPictureModels.Add(new PictureModel
                {
                    ImageUrl = _pictureService.GetPictureUrl(c, 100),
                    Title = c.TitleAttribute,
                    AlternateText = c.AltAttribute
                });
            }

            // brand
            var brand = _brandService.GetBrandById(product.BrandId);
            var brandPic = _pictureService.GetPictureById(brand.PictureId);
            model.BrandPictureModel = new PictureModel
            {
                ImageUrl = _pictureService.GetPictureUrl(brandPic, 100),
                Title = string.Format("{0} Moveleiros", brand.Name),
                AlternateText = string.Format("{0} Moveleiros", brand.Name)
            };
            model.Brand = brand;
            ...
            productStore = productStore == 0 ? 1 : productStore;
            
            var mappedStore = storeService.GetStoreById(productStore);
            var logoPictureId = mappedStore?.LogoPictureId ?? 0;

            model.SellerStoreName = mappedStore.CompanyName;

            var storeCity = cityService.GetCityById(mappedStore.CityId);

            if (storeCity != null)
                model.StoreShortAddress = $"{storeCity.Description} - {storeCity.UF}";

            model.LogoPictureModel = new PictureModel
            {
                ImageUrl = _pictureService.GetPictureUrl(logoPictureId, 100),
                Title = mappedStore?.CompanyName ?? "",
                AlternateText = mappedStore?.CompanyName ?? ""
            };

            // dimension
            if (product.DimensionImageId.HasValue)
            {
                var dimenPic = _pictureService.GetPictureById(product.DimensionImageId.Value);
                model.DimensionPicture = new PictureModel
                {
                    ImageUrl = _pictureService.GetPictureUrl(dimenPic, 512),
                    Title = string.Format("{0} Moveleiros", brand.Name),
                    AlternateText = string.Format("{0} Moveleiros", brand.Name)
                };

                model.DefaultDimensionPicture = new PictureModel
                {
                    FullSizeImageUrl = _pictureService.GetPictureUrl(dimenPic, 0),
                    Title = string.Format("{0} Moveleiros", brand.Name),
                    AlternateText = string.Format("{0} Moveleiros", brand.Name)
                };
            }
